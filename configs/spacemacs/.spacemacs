...
It should only modify the values of Spacemacs settings."
  ...
  (setq-default
   ...
   dotspacemacs-themes '((kanagawa-dragon :package kanagawa-themes))
   ...
   ;; (default "M-m")
   dotspacemacs-emacs-leader-key "C-c SPC"
   ...
   ;; virtual indentation of simple modes. This can interfee with mode specific
  ...
  (with-eval-after-load 'org
    (setq org-directory "~/org"
          ...
          org-agenda-files (mapcar (lambda (f) (expand-file-name f org-directory))
                                   '("inbox.org" "gtd.org" "research.org" "someday.org")))
    ...
    (setq org-capture-templates
          '(("t" "Todo (inbox)" entry
            ...
            ("n" "Note (inbox)" entry
             (file+headline "~/org/inbox.org" "Inbox")
             "* %?\n  %U\n  %a")
            ("p" "Paper note (research)" entry
             (file+headline "~/org/research.org" "Papers")
             "* %^{Title}\n  %U\n  %a\n  - Summary: %?\n  - Key points:\n  - TODO:\n")
            ("s" "Someday" entry
             (file+headline "~/org/someday.org" "Someday")
             "* %?\n  %U\n  %a")))
